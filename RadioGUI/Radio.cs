...
namespace RadioControllerApp
{
    public class FormRadio : Form
    {
        ...
        private bool RecordToFile = false;
        ...
        private Dictionary<string, int> toneMappings = new Dictionary<string, int>
{
    ...
    { "250.3", 38 }
        ...
        private GroupBox CreateConnectionControls()
        {
            ...
            btnStartRecording = new Button { Text = "if red will\nrecord to\n ~/kv4p_cs.pcm", AutoSize = true }; //ddd

            btnOpenConnection.Click += btnOpenConnection_Click;
            btnCloseConnection.Click += btnCloseConnection_Click;

            btnStartRecording.Click += btnStartRecording_Click; //ddd


            layout.Controls.Add(lblPortName, 0, 0);
            layout.Controls.Add(cmbPortName, 1, 0);
            layout.Controls.Add(btnOpenConnection, 3, 0);
            layout.Controls.Add(btnCloseConnection, 4, 0);
            layout.Controls.Add(btnStartRecording, 2, 0); //ddd
        ...
        private GroupBox CreateFrequencyControls()
        {
            ...
            //ddd
            Label lblBW = new Label { Text = "BW:", Anchor = AnchorStyles.Right };
            cmbBW = new ComboBox { DropDownStyle = ComboBoxStyle.DropDownList, Anchor = AnchorStyles.Left };
            ...
            //ddd
            layout.Controls.Add(lblBW, 6, 0);
            layout.Controls.Add(cmbBW, 7, 0);
            ...
            cmbBW.Items.Add("N");
            cmbBW.Items.Add("W");
            cmbBW.SelectedIndex = 0; // Default to "N:
            for (int i = 0; i <= 9; i++)
            {
        ...
        private void InitializeWaveOut()
        {
            ...
            //receivedAudioBuffer = new BufferedWaveProvider(new WaveFormat(22050, 8, 1))
            receivedAudioBuffer = new BufferedWaveProvider(new WaveFormat(16000, 8, 1))
            {
        ...
        private void btnOpenConnection_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (selectedPortInfo != null)
                {
                    ...
                    radioController.RecordToFile = RecordToFile; //ddd
        ...
        private void btnTune_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // ddd Get selected BW
                string BW = cmbBW.SelectedItem.ToString();
               
                // Get selected squelch level
                int squelch = int.Parse(cmbSquelchLevel.SelectedItem.ToString());

                radioController.TuneToFrequency(txFreq, rxFreq, toneValue, squelch, BW);
                AppendStatus($"Tuned to TX: {txFreq}, RX: {rxFreq}, Tone: {selectedTone}, Squelch Level: {squelch}, BW: {BW}");
            }
            catch (Exception ex)
            {
        ...
        {
            btnStartRecording.BackColor = (RecordToFile ? Color.White : Color.Red);
